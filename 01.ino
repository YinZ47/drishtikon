/*
  ...
  - I2C updates only when the pulse actually changes to reduce bus jitter

  Note on the original error:
  - “T does not name a type” was caused by a templated clamp<T> function in a .ino file.
    Arduino’s preprocessor inserts prototypes that break templates. This refactor removes templates.
*/
...
void computeTimings(float bpm) {
  ...
  uint32_t avDelay = (uint32_t)clampf(0.12f * cycleMs, 80.0f, 180.0f);

  // Atrial contraction 90–200 ms
  uint32_t atriaContract = (uint32_t)clampf(0.18f * cycleMs, 90.0f, 200.0f);

  // Ventricular systole total ~ 0.3–0.4 of cycle, but clamped
  uint32_t ventSystole = (uint32_t)clampf(0.35f * cycleMs, 260.0f, 380.0f);

  // Break ventricular systole into segments
  uint32_t ventContract = (uint32_t)clampf(0.40f * ventSystole, 120.0f, 220.0f);
  uint32_t ventHold     = (uint32_t)clampf(0.20f * ventSystole,  70.0f, 150.0f);
  uint32_t ventRelax    = (uint32_t)clampf(0.40f * ventSystole, 120.0f, 220.0f);

  // Atria relax starts a bit after vent starts contracting (fixed 60 ms after AV delay)
  uint32_t atriaRelaxStart = avDelay + 60;
  uint32_t atriaRelax      = (uint32_t)clampf(0.22f * cycleMs, 110.0f, 220.0f);
...
void setup() {
  ...
  Wire.begin();
  Wire.setClock(400000);     // If your board doesn't support this, comment it out.
  pwm.begin();
