/*
  Nilheim Mechatronics Servo Heart – biologically sequenced driver (rev B)
  Requires: Adafruit PWM Servo Driver Library
  https://github.com/adafruit/Adafruit-PWM-Servo-Driver-Library

  Changes in rev B:
  - Power-on soft-start: 1.5 s supply settle + ~1.2 s amplitude ramp for first beats
  - Enforced atria→AV buffer→ventricular sequence even when BPM changes
  - Guaranteed diastolic reserve per cycle (prevents “no-fill” at high BPM)
  - I2C update deadband to reduce jitter
  - Safer Wire.setClock use
*/
...
// I2C update deadband (PWM counts 0..4095); avoid spam for ±1-count noise
constexpr uint16_t PWM_CHANGE_DEADBAND = 2;
...
void computeTimings(float bpm) {
  ...
  // Atrial contraction 100–180 ms, but ensure it finishes before vent starts
...
// Compute amplitude scale during soft-start (ramps 0→1)
...
void updateHeart(uint32_t nowMs) {
  ...
  // Apply soft-start amplitude to atria excursion
  atriaAngle = a_relaxed + (atriaAngle - a_relaxed) * amp;
  ...
  // Apply soft-start amplitude to vent excursion
...
void setup() {
  ...
  Serial.begin(115200);
  delay(50);
  Serial.println("Nilheim Heart – biologically sequenced servo driver (rev B)");
  ...
  // Give power rails time to settle before we hold any position
