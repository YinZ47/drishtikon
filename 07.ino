/*
  ...
  (rev D″ final: tuned motion curves, same safe ranges & wiring)

  What changed vs your D′:
  - Kept all "sensitive" values as-is (BPM range, delays, safety windows).
  - Atria now use: contraction = ease-in cubic, relaxation = ease-out cubic.
  - Ventricle now uses: contraction = ease-in cubic, relaxation = ease-out quint (longer tail).
  - Same soft-start, diastolic guard, EMA pot filtering, I2C deadband, slew limits, and safety rails.
*/

#include <Wire.h>
#include <Adafruit_PWMServoDriver.h>
#include <math.h>   // isfinite, fabsf, powf
...
// -------- VENTRICLE CALIBRATION (kept from your D′) --------
...
ServoCal cal[3] = {
  ...
  // Atria R
  { 140, 520,  25.0f,  55.0f },
  // Ventricle (base 90°, peak 130°; hard clamp 70..130 enforced below)
...
void writeServoAngle(uint8_t servoIdx, float angleDeg) {
  ...
  // For the ventricle, enforce absolute safe window 70..130 exactly
  ...
  // Enforce angular velocity limits (gentle for all)
...
void updateHeart(uint32_t nowMs) {
  ...
  // ---------- VENTRICLE (rapid upstroke, gentle release; clamped 70..130) ----------
  ...
  // Contract phase: 90 → 130 with fast upstroke
  ...
  // Relax phase: 130 → 90 with longer tail
  float v_down = easeOutQuint( linProgress(t, v_t2, timing.ventRelaxMs) );
  if (t >= v_t2) {
    ventAngle = v_contr + (v_relaxed - v_contr) * v_down;
  }

  // Soft-start amplitude scales excursion around base (holds base at 90°)
...
void setup() {
  ...
  Serial.begin(115200);
  delay(50);
  Serial.println("Nilheim Heart – rev D″ (final tuned curves, PCA9685 all, vent 90° base / 70–130° clamp)");
