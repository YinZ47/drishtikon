/*
  Bio-tuned Eye Controller (ESP32 / AVR)  -- FINAL
  - Same servo geometry/timings and protocol as your version.
  - Added: accepts "SCAR ON|OFF" and replies "OK" (ignored otherwise).
  - Parser slightly more tolerant. Everything else untouched.
  ...
  Protocol:
    ...
    PING
    HELP
    SCAR ON|OFF  (no-op placeholder; returns OK)

  Notes:
   - Ensure solid 5–6V servo power, common ground with MCU.
   - PCA9685 address default 0x40; change if your board differs.
*/

#define PCA9685_ADDR        0x40
//#define PCA9685_EXT_OSC_HZ  27000000UL
//#define DEBUG_PRINTS
...
void moveServoSmooth(uint8_t ch, int &cur, int targetAngle, int step=PROFILE_STEP_DEG, int stepDelay=MIN_STEP_DELAY_MS){
  ...
  if (step<1) step=1;
  if (cur==targetAngle){ writeAngle(ch,cur); return; }
  ...
  for(int a=cur; a!=targetAngle; a+=dir*step){
    ...
    writeAngle(ch,a); cur=a; SLEEP_MS(stepDelay); MAYBE_YIELD();
    if (a==targetAngle) break;
  }
}
struct ServoState{ uint8_t ch; int* currentRef; int target; };
...
void moveServosSimultaneous(ServoState* list, int count, int step=PROFILE_STEP_DEG, int stepDelay=MIN_STEP_DELAY_MS){
  ...
  while(any){
    ...
    for(int i=0;i<count;++i){
      int &cur=*(list[i].currentRef); int tgt=constrain(list[i].target,0,180);
      if(cur==tgt) continue;
      int delta=tgt-cur; int inc=constrain(delta,-step,step); int next=cur+inc;
      writeAngle(list[i].ch,next); cur=next; any=true;
    }
    if(any){ SLEEP_MS(stepDelay); MAYBE_YIELD(); }
  }
}
...
int lerpInt(int a,int b,float t){ float ft=a+(b-a)*t; return (int)(ft + (ft>=0?0.5f:-0.5f)); }
...
void setBothEyelidsOpenLevel(float open01,int step=PROFILE_STEP_DEG,int stepDelay=MIN_STEP_DELAY_MS){
  ...
  int tgt_LU=lerpInt(LU_max_close,LU_max_open,open01)+trim_LU;
  int tgt_LD=lerpInt(LD_max_close,LD_max_open,open01)+trim_LD;
  ServoState g[4]={{RU,&currentAngle_RU,tgt_RU},{RD,&currentAngle_RD,tgt_RD},
                   {LU,&currentAngle_LU,tgt_LU},{LD,&currentAngle_LD,tgt_LD}};
...
void saccadeToNormalized(float xNorm,float yNorm){
  xNorm=constrain(xNorm,-1.0f,1.0f); yNorm=constrain(yNorm,-1.0f,1.0f);
  int tgtMF=hNormToMF(xNorm); int tgtMB=vNormToMB(yNorm);
  int dMF=abs(tgtMF-currentAngle_MF); int dMB=abs(tgtMB-currentAngle_MB);
...
void blinkBothBio(){
  ...
  if(!lidsOnly){
    ...
    if(bellTarget!=originalMB){
      ...
      uint16_t tBell=(uint16_t)max(30,(int)tClose-20);
  ...
  int tRUc=RU_max_close+trim_RU, tRDc=RD_max_close+trim_RD, tLUc=LU_max_close+trim_LU, tLDc=LD_max_close+trim_LD;
  int dRU=abs(tRUc-currentAngle_RU), dRD=abs(tRDc-currentAngle_RD), dLU=abs(tLUc-currentAngle_LU), dLD=abs(tLDc-currentAngle_LD);
  int dMax=myMax(myMax(dRU,dRD),myMax(dLU,dLD));
  int stepClose=computeStepDelayForDurationMs(dMax,tClose,PROFILE_STEP_DEG);
  ServoState closeGroup[4]={{RU,&currentAngle_RU,tRUc},{RD,&currentAngle_RD,tRDc},{LU,&currentAngle_LU,tLUc},{LD,&currentAngle_LD,tLDc}};
  moveServosSimultaneous(closeGroup,4,PROFILE_STEP_DEG,stepClose);

  SLEEP_MS(tHold);

  int tRUo=RU_max_open+trim_RU, tRDo=RD_max_open+trim_RD, tLUo=LU_max_open+trim_LU, tLDo=LD_max_open+trim_LD;
  dRU=abs(tRUo-currentAngle_RU); dRD=abs(tRDo-currentAngle_RD); dLU=abs(tLUo-currentAngle_LU); dLD=abs(tLDo-currentAngle_LD);
  dMax=myMax(myMax(dRU,dRD),myMax(dLU,dLD));
  int stepOpen=computeStepDelayForDurationMs(dMax,tOpen,PROFILE_STEP_DEG);
  ServoState openGroup[4]={{RU,&currentAngle_RU,tRUo},{RD,&currentAngle_RD,tRDo},{LU,&currentAngle_LU,tLUo},{LD,&currentAngle_LD,tLDo}};
  ...
  // next blink scheduled by loop timing
}
...
void printHelp(){
  ...
  Serial.println(F("  RAW mf mb     (direct degrees)"));
  Serial.println(F("  STATUS"));
  Serial.println(F("  PING"));
  Serial.println(F("  SCAR ON|OFF   (no-op)"));
...
void setHostMode(bool on){
  ...
  if(on){ scheduleNextBlink(); }
  else  { scheduleNextBlink(); scheduleNextGaze(); scheduleNextMicro(); }
  Serial.print(F("Mode: ")); Serial.println(on?F("HOST"):F("AUTO"));
}
void handleSTATUS(){
  ...
  Serial.print(F("RU/RD/LU/LD: ")); Serial.print(currentAngle_RU); Serial.print(' ');
  Serial.print(currentAngle_RD); Serial.print(' '); Serial.print(currentAngle_LU); Serial.print(' ');
...
void processCommand(char* line){
  ...
  if(*line==0) return;
  char buf[CMD_BUF_LEN]; strncpy(buf,line,CMD_BUF_LEN-1); buf[CMD_BUF_LEN-1]=0;
  char* saveptr=nullptr; char* tok=strtok_r(buf," \t",&saveptr);
  if(!tok) return;
  ...
  if(!strcmp(tok,"MODE")){
    ...
    saccadeToNormalized(x,y); hostLastCmdAt=millis();
  }
  else if(!strcmp(tok,"CENTER")){
    if(!hostControlEnabled) setHostMode(true);
    saccadeToNormalized(0.0f,0.0f); hostLastCmdAt=millis();
  }
  else if(!strcmp(tok,"BLINK")){
    blinkBothBio(); hostLastCmdAt=millis();
  }
  else if(!strcmp(tok,"OPEN")){
    char* a=strtok_r(nullptr," \t",&saveptr);
    if(!a){ Serial.println(F("ERR: OPEN requires 0..1")); return; }
    float o=(float)strtod(a,nullptr); o=clamp01(o);
    setBothEyelidsOpenLevel(o,PROFILE_STEP_DEG,8); hostLastCmdAt=millis();
  }
    ...
    int mfA=atoi(mf); int mbA=atoi(mb);
    if(!hostControlEnabled) setHostMode(true);
    set_MF(mfA,PROFILE_STEP_DEG,6); set_MB(mbA,PROFILE_STEP_DEG,6);
  }
  ...
  else if(!strcmp(tok,"SCAR")){
    // harmless placeholder for Python side experiments
    Serial.println(F("OK"));
  }
    ...
    Serial.print(F("ERR: Unknown cmd: ")); Serial.println(tok);
  }
}
...
#endif
  Serial.print(F("Starting Bio Eye Controller on ")); Serial.println(F(PLATFORM_NAME));
...
void loop(){
  ...
  if(hostControlEnabled){
    if((int32_t)(now - nextBlinkAt)>=0){ blinkBothBio(); scheduleNextBlink(); }
  } else {
    if((int32_t)(now - nextBlinkAt)>=0){ blinkBothBio(); scheduleNextBlink(); }
    if((int32_t)(now - nextGazeAt)>=0){ float gx,gy; chooseNewGazeTarget(gx,gy); saccadeToNormalized(gx,gy); scheduleNextGaze(); }
    if((int32_t)(now - nextMicroAt)>=0){ doMicroSaccade(); scheduleNextMicro(); }
  }

  SLEEP_MS(5); MAYBE_YIELD();
}
