#!/usr/bin/env python3
"""
Eye Host (REF-FAST + Heart Sync)
- Faster face pipeline: tracker + Haar fast path, YOLO only as needed.
- Optional second serial port to Heart Arduino (HR sync with GUI BPM).
- Keeps your Eye MCU protocol exactly the same.

Keys:
  t TRACK | a AUTO | m MANUAL | g auto-gamma
  x/y invert | b blink | o open | c close | arrows (manual)
  h heart window toggle | 0 none | 1 brady | 2 tachy | 3 af | q quit
"""
...
# ===== Try Ultralytics (YOLOv8). OK if missing (we have fast path) =====
try:
...
WINDOW_NAME = "Face Tracking (FAST pipeline)"
CTRL_WIN    = "Controls"
    ...
    if preferred:
        for p in ports:
            if preferred.lower() in (p.description or "").lower() or preferred.lower() in p.device.lower():
                return p.device
    # silly but practical heuristics
    for p in ports:
        desc = (p.description or "").lower()
        dev = p.device
        if exclude_set and dev in exclude_set:  # avoid picking the eye port
            continue
...
def load_models(face_model, generic_model, allow_fallback=True, debug=False):
    have_face_model = False
    model = None
    haar = None
    if face_model and Path(face_model).exists() and YOLO is not None:
        model = YOLO(face_model)
        have_face_model = True
        print(f"[INFO] Using face model: {face_model}")
    elif allow_fallback and YOLO is not None:
        print("[WARN] Face model missing or YOLO not available. Falling back to generic YOLO and Haar.")
        model = YOLO(generic_model) if Path(generic_model).exists() else None
    else:
        print("[INFO] DNN disabled (fast path).")
    if ensure_haar(debug=debug):
        haar = cv2.CascadeClassifier(HAAR_FILE)
        if haar.empty():
            print("[ERROR] Haar cascade load failed.")
            haar = None
    return model, haar, have_face_model

# ------------------ Simple tracker helpers -------------------
def create_tracker():
    tr = None
    # Prefer MOSSE for speed, fallback KCF/MIL if missing
    try:
...
def detect_face_adaptive(proc_frame, model, have_face_model, haar, conf, area_priority,
                         last_box, miss_streak, frame_idx, img_size,
                         prefer_haar_fast=False, allow_rotation=True,
                         tracker=None, tracker_iou_gate=0.15,
                         det_interval=3, do_fullframe=True):
    """Returns: cx,cy,W,H,box,conf,angle_used"""
    h_full,w_full=proc_frame.shape[:2]

    # 0) Tracker first (cheap)
    if tracker is not None:
        ok, tb = tracker.update(proc_frame)
        if ok:
            x,y,w,h = [int(v) for v in tb]
            boxT = (x,y,x+w,y+h)
            if last_box is None or iou(boxT, last_box) >= tracker_iou_gate:
                cx = x + w//2; cy = y + h//2
                return cx, cy, w_full, h_full, boxT, 0.95, 0.0
        # tracker failed → drop it
    # 1) ROI fast pass
    if last_box and miss_streak==0 and frame_idx % ROI_REFRESH_INTERVAL != 0 and not prefer_haar_fast:
        ...
        sub=proc_frame[ry1:ry2,rx1:rx2]
        if model is not None:
            sub_size=min(img_size,max(sub.shape[:2]))
            r=model(sub, imgsz=sub_size, verbose=False)[0]
            if have_face_model:
                cx0,cy0,_,_,b0,cf0=pick_face_yolo(r,conf,area_priority)
            else:
                cx0,cy0,_,_,b0,cf0=detect_person_face_with_haar(r,haar,conf,area_priority)
            if cx0 is not None:
                bx1,by1,bx2,by2=b0
                return int(cx0+rx1),int(cy0+ry1),w_full,h_full,(bx1+rx1,by1+ry1,bx2+rx1,by2+ry1),cf0,0.0

    # 2) Haar-only fast path (optional; very fast on CPU)
    ...
    # 3) Periodic full-frame DNN (rate-limited)
    do_det = (frame_idx % det_interval == 0) or (miss_streak > 0) or not do_fullframe
    if model is not None and do_det:
        res=model(proc_frame, imgsz=img_size, verbose=False)[0]
        if have_face_model:
    ...
    # 4) Rotation fallback (throttled by caller)
    if allow_rotation and miss_streak>0 and model is not None:
        ...
        for ang in angles:
            ...
            if have_face_model:
                rx,ry,_,_,rbox,rconf=pick_face_yolo(rres,conf,area_priority)
            else:
                rx,ry,_,_,rbox,rconf=detect_person_face_with_haar(rres,haar,conf,area_priority)
            if rx is None: continue
            obox=map_box_back(rbox,Minv)
...
def init_controls():
    ...
    cv2.createTrackbar("Img Size", CTRL_WIN, 512, 1280, lambda v: None)  # slightly smaller default
...
def get_ctrl_values(state: State, args):
    ...
    send_interval = max(MIN_SEND_INTERVAL*1000, send_ms) / 100.0 / 10.0  # keep compatible
    img_size = max(160, img_size)
    return gamma, eyelid_open_pct/100.0, deadzone_pct/100.0, send_interval, img_size

# ================== Heart monitor pieces (unchanged logic) ==================
...
# ------------------ Parser -------------------
def build_parser():
    p = argparse.ArgumentParser(description="FAST Face Tracking -> Eye MCU, Heart sync.")
    # eye
    ...
    p.add_argument("--fast", action="store_true", help="Prefer Haar-only detection + tracker; skip DNN unless needed.")
    p.add_argument("--use-tracker", action="store_true", default=True)
    p.add_argument("--det-interval", type=int, default=3, help="Do DNN every N frames (with tracker).")
...
def main(args):
    ...
    if args.heart_port:
        ...
        if args.heart_port.upper()=="AUTO":
            # try to avoid picking the eye's port
    ...
    rotation_cooldown = 0  # rate-limit rotation fallback
    ...
    while state.running:
        ...
        if state.mode_manual:
            ...
            cx, cy, Fw, Fh, box, conf, angle_used = detect_face_adaptive(
                proc_frame=proc,
                model=model,
                have_face_model=have_face_model,
                haar=haar,
                conf=args.conf,
                ...
                tracker_iou_gate=state.tracker_iou_gate,
            ...
            if detected:
                ...
                if use_tracker:
                    if state.tracker is None or state.last_box is None or iou(box, state.last_box) < 0.1:
                        tr = create_tracker()
                        if tr is not None:
                            tr.init(proc, (box[0], box[1], box[2]-box[0], box[3]-box[1]))
                        state.tracker = tr
            ...
            # AUTO mode: MCU wanders eye; we just show gamma/fps and drive HR gently
...
def maybe_tx_hr(state: State, bpm_val: float, hz: float):
    # Transmit rate-limited integer BPM if changed or interval elapsed
    now = time.time()
