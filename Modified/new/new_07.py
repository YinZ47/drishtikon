...
- SCARED requires sustained fast motion: 75th-percentile speed >= 1.20 norm units/s over 0.45 s.
- IN MOTION when 75th-percentile speed >= 0.60 over 0.25 s.
...
class VelocityTracker:
    ...
    - Uses 75th-percentile speed within windows:
        * fast_window=0.45 s -> threshold abs_fast_thr=1.20 (SCARED)
        * move_window=0.25 s -> threshold abs_move_thr=0.60 (IN MOTION)
    - Ignores tiny jitter via pos_epsilon.
    """
    def __init__(self,
                 fast_window=0.45, move_window=0.25,
                 abs_fast_thr=1.20, abs_move_thr=0.60,
                 pos_epsilon=0.02,
                 min_fast_samples=5, min_move_samples=3,
                 warmup_sec=0.6):
...
def main(args):
    ...
    # Heart machinery with Option B thresholds
    vel = VelocityTracker(
        fast_window=0.45, move_window=0.25,
        abs_fast_thr=1.20, abs_move_thr=0.60,
        pos_epsilon=0.02, min_fast_samples=5, min_move_samples=3,
        warmup_sec=0.6
    )
