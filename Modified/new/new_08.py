#!/usr/bin/env python3
"""
Eye Host (Stable HUD + Face-Only Anti-Drift + Heart Sync + Instant-Speed SCARED/STABILIZING)

- Face-only tracking (YOLO face -> DNN -> Haar) with tracker verification (prevents chest drift).
- Stable HUD always drawn.
- Heart monitor (ECG + BPM) tied to motion; optional serial sync (HR / HRMODE) to heart MCU.
- SCARED uses **instantaneous** face speed (no delay): default fast_thr=0.65 norm units/s.
- IN MOTION when instantaneous speed >= 0.40 norm units/s.
...
# -------------- Tracking -----------------
def create_tracker():
    # Try CSRT then MOSSE (with/without legacy namespace)
    for maker in [
...
class VelocityTracker:
    """
    Instantaneous-speed classifier (no delay window).
    - raw speed = sqrt(dx^2 + dy^2) / dt (normalized units/s)
    - SCARED if raw >= fast_thr
    - IN MOTION if raw >= move_thr
    - Otherwise Stable
    """
    def __init__(self, fast_thr=0.65, move_thr=0.40, pos_epsilon=0.02, ema_alpha=0.20, warmup_sec=0.3):
        self.fast_thr=float(fast_thr)
        self.move_thr=float(move_thr)
        self.pos_eps=float(pos_epsilon)
    ...
    def update(self, cx_norm, cy_norm, tnow):
        ...
        if self.last is not None and self.last_t is not None:
            ...
            if abs(dx)<self.pos_eps: dx=0.0
            if abs(dy)<self.pos_eps: dy=0.0
            raw=(dx*dx+dy*dy)**0.5 / dt
        # small EMA for display only (decision uses raw)
        self.ema = raw if self.ema is None else (self.ema_alpha*raw + (1-self.ema_alpha)*self.ema)
        ready = tnow >= self.warmup_end

        if raw >= self.fast_thr:
            self.state = "Moving fast"
        elif raw >= self.move_thr:
            self.state = "In motion"
        else:
            self.state = "Stable"

        self.last=(cx_norm,cy_norm); self.last_t=tnow
        # return: raw, ema, state, ready, base, noise (base/noise repurposed here)
        return raw, (self.ema if self.ema is not None else raw), self.state, ready, self.ema or raw, 0.0

class HeartRateModel:
    def __init__(self, disease="none"):
        self.set_disease(disease)
        self.hr=float(self.base_bpm); self.scare_bump=0.0
        self.last_t=time.time(); self.stable_accum=0.0; self.cooldown_hold=0.8
        self.prev_hr=None
    ...
    def update(self, vstate):
        ...
        # Fast reaction on SCARED, modest on motion, decay on stable
        if vstate=="Moving fast": target_bump=24.0; tau=0.7
        elif vstate=="In motion": target_bump=8.0;  tau=1.0
        else: target_bump=0.0;   tau=2.0
...
# -------------- CLI ----------------------
def build_parser():
    p=argparse.ArgumentParser(description="Stable HUD face tracking + heart sync -> Arduino (instant speed)")
    # eye
    ...
    # motion thresholds
    p.add_argument("--fast-thr", type=float, default=0.65, help="Instant speed threshold for SCARED")
    p.add_argument("--move-thr", type=float, default=0.40, help="Instant speed threshold for IN MOTION")
    return p
...
def main(args):
    ...
    # Heart machinery with instant-speed thresholds
    vel = VelocityTracker(fast_thr=args.fast_thr, move_thr=args.move_thr, pos_epsilon=0.02, ema_alpha=0.20, warmup_sec=0.3)
    ...
    while state.running:
        ...
        if state.mode_manual:
            ...
            if box is not None and cx is not None and cy is not None:
                ...
                # temporal centroid smoothing (for drawing)
                def tavg(seq, v, w=3):
                    seq.append(v)
                ...
                # instant-speed motion classification
                raw_spd, ema_spd, vstate, ready, _, _ = vel.update(x_norm_raw, y_norm_raw, time.time())
