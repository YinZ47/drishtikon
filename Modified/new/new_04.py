#!/usr/bin/env python3
"""
Eye Host (Stable HUD + Face-Only Anti-Drift + Heart Sync + Stronger Motion/SCARED)
...
- More sensitive VelocityTracker with absolute-speed fallback to trigger SCARED reliably.
...
class VelocityTracker:
    ...
    Tuned to be more sensitive and includes absolute-speed fallback.
    """
    def __init__(self, alpha=0.20, warmup_sec=1.5,
                 # adaptive thresholds (multiples of noise)
                 k1=2.4, k2=6.0,
                 # dwell times
                 dwell_move_on=0.18, dwell_move_off=0.15,
                 dwell_fast_on=0.20, dwell_fast_off=0.20,
                 # absolute-speed fallback (units/sec in normalized coords)
                 abs_move_thr=0.35, abs_fast_thr=0.80):
    ...
    def update(self, cx_norm, cy_norm, tnow):
        ...
        if not ready:
            ...
            self.baseline,self.noise=self._rob(); self.hyst_eps=0.20*self.noise
        else:
...
class HeartRateModel:
    ...
    def update(self, vstate):
        ...
        if vstate=="Moving fast": target_bump=26.0; tau=0.9
        elif vstate=="In motion": target_bump=11.0; tau=1.2
        else: target_bump=0.0; tau=2.6
        self.scare_bump += (target_bump - self.scare_bump) * (1.0 - np.exp(-dt/tau))
        self.scare_bump = float(np.clip(self.scare_bump,0.0,28.0))
        target = self.base_bpm + self.scare_bump
        if vstate=="Stable" and self.stable_accum<self.cooldown_hold: target=self.hr
        tau_up=0.9; tau_dn=7.0
    ...
    while state.running:
        ...
        if state.mode_manual:
            ...
            if box is not None and cx is not None and cy is not None:
                ...
                # motion update (more sensitive than before)
