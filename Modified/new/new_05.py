#!/usr/bin/env python3
"""
Eye Host (Stable HUD + Face-Only Anti-Drift + Heart Sync + Calibrated SCARED/STABILIZING)

- Robust face-only tracking (YOLO face -> DNN -> Haar).
- Stable HUD always visible.
- Heart monitor (ECG + BPM) tied to motion; serial sync to heart MCU (HR / HRMODE).
- Calibrated motion classifier:
    * SCARED only on genuinely fast movement.
    * STABILIZING shown while HR decays back to baseline when stable.
...
class VelocityTracker:
    """
    Detects motion state from normalized gaze center speed.
    Calibrated to avoid false SCAREDs from noise.
    """
    def __init__(self, alpha=0.15, warmup_sec=2.2,
                 # adaptive thresholds (multiples of noise)
                 k1=3.5, k2=9.0,
                 # dwell times
                 dwell_move_on=0.30, dwell_move_off=0.30,
                 dwell_fast_on=0.32, dwell_fast_off=0.30,
                 # absolute-speed fallback (units/sec in normalized coords)
                 abs_move_thr=0.60, abs_fast_thr=1.30,
                 # ignore tiny position jitter
                 pos_epsilon=0.02):
        ...
        self.abs_move_thr=abs_move_thr; self.abs_fast_thr=abs_fast_thr
        ...
        self.last_raw=0.0; self.last_ema=0.0
    ...
    def update(self, cx_norm, cy_norm, tnow):
        ...
        # absolute-speed fallback augments the flags
        above_fast = (self.ema>thr_f+self.hyst_eps) or (self.ema>=self.abs_fast_thr)
        above_move = (self.ema>thr_m+self.hyst_eps) or (self.ema>=self.abs_move_thr)
        ...
        if self.state=="Stable":
            ...
            if self._below_fast and (now-self._below_fast)>=self.dwell_fast_off:
                # Drop to "In motion" if still above move threshold; else Stable.
                self.state="In motion" if above_move else "Stable"
                self._below_fast=None

        self.last=(cx_norm,cy_norm); self.last_t=tnow
        self.last_raw=raw; self.last_ema=float(self.ema)
...
class HeartRateModel:
    def __init__(self, disease="none"):
        ...
        self.last_t=time.time(); self.stable_accum=0.0; self.cooldown_hold=1.4
        self.prev_hr=None
    ...
    def update(self, vstate):
        ...
        if vstate=="Stable" and self.stable_accum<self.cooldown_hold:
            target=self.hr  # short hold before decaying
        tau_up=0.9; tau_dn=5.5  # slightly faster recovery
...
def main(args):
    ...
    while state.running:
        ...
        if state.mode_manual:
            draw_hud(display, "Mode: MANUAL (HOST)", fps_show, state.gamma, state.invert_x, state.invert_y, motion_line)
            # keep heart alive as "Stable"
            if state.show_heart and hr_ui is not None:
                bpm,_ = hr_model.update("Stable")
                v = ecg.step(bpm)
                # show stabilizing if decaying toward base
                status = "STABILIZING" if (hr_model.prev_hr is not None and hr_model.prev_hr - bpm > 0.15 and bpm > hr_model.base_bpm + 2.0) else None
            ...
            if box is not None and cx is not None and cy is not None:
                ...
                # motion update (calibrated)
                raw_spd, ema_spd, vstate, ready, base, noise = vel.update(x_norm_raw, y_norm_raw, time.time())
                if state.show_motion_line:
                    motion_line=f"spd raw:{raw_spd:.2f} ema:{ema_spd:.2f} noise:{noise:.3f} state:{vstate}"
                    draw_hud(display, "Mode: TRACK (HOST)", fps_show, state.gamma, state.invert_x, state.invert_y, motion_line)
                ...
                # HEART: reflect motion
                bpm, scared = hr_model.update(vstate)
                v = ecg.step(bpm)
                status = "SCARED" if scared else None
                # show STABILIZING when stable and HR decays toward baseline
                if vstate=="Stable" and (hr_model.prev_hr is not None) and (hr_model.prev_hr - bpm > 0.15) and (bpm > hr_model.base_bpm + 2.0):
                    status = "STABILIZING"
                if state.show_heart and hr_ui is not None:
                    hr_ui.render(bpm, v, fps_show, status=status)
                if heart_enabled:
            ...
            if state.show_heart and hr_ui is not None:
                hr_ui.render(bpm, v, fps_show, status=status)
            # optional: push baseline BPM to heart MCU
            maybe_tx_hr(state, bpm, args.heart_tx_hz)
